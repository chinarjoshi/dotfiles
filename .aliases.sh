alias hnix='hx /etc/nixos/configuration.nix' ## hx
...
alias gl="git log --color --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset' --abbrev-commit" ## git
alias gm='git merge' ## git
alias halias='hx ~/.aliases.sh' ## hx
...
alias advos='TERM=xterm ssh advos' ## xterm
